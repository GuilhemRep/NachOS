...
void ExceptionHandler(ExceptionType exceptiontype, int vaddr)
{
  ...
  case SYSCALL_EXCEPTION: {
	...
	int sem_id = g_machine->ReadIntRegister(10); // Take the first parameter.
	// On récupère le sémaphore associé.
	Semaphore* sema = (Semaphore*) g_object_addrs->SearchObject(sem_id);
	if (sema && sema->type == SEMAPHORE_TYPE) {
		// Le sémaphore existE et est valide.
		sema->P();
		g_machine->WriteIntRegister(10, NO_ERROR);
		g_syscall_error->SetMsg((char*)"",NO_ERROR);
	} else {
		// Le sémaphore n'existe pas ou n'est pas bon...
	...
	break;
     ...
     case SC_SEM_CREATE: {
	DEBUG('e', (char*) "Semaphore creation initiated by user program.\n");
	int addr = g_machine->ReadIntRegister(10); // Take the first parameter.
	int size = GetLengthParam(addr);
	char ch[size];
	GetStringParam(addr,ch,size);
	// Take the second param.
	int count = g_machine->ReadIntRegister(11);

	// On crée le sémaphore.
	Semaphore* sema = new Semaphore(ch, count);

	// On l'ajoute dans les objets.
	int32_t sid = g_object_addrs->AddObject(sema);
	g_syscall_error->SetMsg((char*)"",NO_ERROR);
	// On retourne son adresse dans le registre.

	g_machine->WriteIntRegister(10, sid);
	break;
    }
     case SC_SEM_DESTROY: {
	DEBUG('e', (char*) "Semaphore destroy initiated by user program.\n");
	int addr = g_machine->ReadIntRegister(10); // Take the first parameter.
	// On récupère notre sémaphore.
	Semaphore* sema = (Semaphore*) g_object_addrs->SearchObject(addr);
	if (sema && sema->type == SEMAPHORE_TYPE) {
		// Le sémaphore existe et est valide.
		g_machine->WriteIntRegister(10,NO_ERROR);
		g_syscall_error->SetMsg((char*)"", NO_ERROR);
	...
	break;
     ...
     case SC_LOCK_CREATE: {
	DEBUG('e', (char*) "Lock create initiated by user program.\n");
	int addr = g_machine->ReadIntRegister(10); // Take the first parameter.
	int size = GetLengthParam(addr);
	char ch[size];
	GetStringParam(addr,ch,size);
	...
	// On l'ajoute dans les objets.
	int32_t lid = g_object_addrs->AddObject(lock);
	g_syscall_error->SetMsg((char*)"", NO_ERROR);
	...
	DEBUG('e', (char*) "Lock destroy initiated by user program.\n");
	int addr = g_machine->ReadIntRegister(10); // Take the first parameter.
	// On récupère notre lock.
	Lock* lock = (Lock*) g_object_addrs->SearchObject(addr);
	if (lock && lock->type == LOCK_TYPE) {
		// Le lock existe et est valide.
		g_machine->WriteIntRegister(10, NO_ERROR);
		g_syscall_error->SetMsg((char*)"", NO_ERROR);
		delete lock;
	} else {
	...
	DEBUG('e', (char*) "Lock acquire call initiated by user program.\n");
	int lock_id = g_machine->ReadIntRegister(10); // Take the first parameter.
	// On récupère le lock associé.
	Lock* lock = (Lock*) g_object_addrs->SearchObject(lock_id);
	if (lock && lock->type == LOCK_TYPE) {
		// Le sémaphore existe et est valide.
		lock->Acquire();
		g_machine->WriteIntRegister(10, NO_ERROR);
		...
		// Le lock n'existe pas ou n'est pas bon...
		g_syscall_error->SetMsg((char*)"", INVALID_LOCK_ID);
		g_machine->WriteIntRegister(10, ERROR);
	}
	break;
	...
	DEBUG('e', (char*) "Lock release call initiated by user program.\n");
	int lock_id = g_machine->ReadIntRegister(10); // Take the first parameter.
	// On récupère le lock associé.
	Lock* lock = (Lock*) g_object_addrs->SearchObject(lock_id);
	if (lock && lock->type == LOCK_TYPE) {
