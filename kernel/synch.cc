...
Semaphore::P() {
  ...
  if (counter < 0) { // On doit bloquer le thread courant.
	// counter = 0 déjà.
	// On ajoute ce thread à la file d'attente des threads waiting_queue.
  	waiting_queue->Append((void *)g_current_thread);
	// Avant de libérer le processeur on remet l'état des interruptions comme précédemment.
	
	g_current_thread->Sleep(); // On met le current thread en état endormi et lâche le processeur.
  }

	g_machine->interrupt->SetStatus(oldLevel);
	// On n'interrompt pas le current thread (dans le sens sleep) dans ce cas.
...
Semaphore::V() {
  ...
  counter += 1;
  if (counter <= 0) { // On a libéré éventuellement un waiting thread.
	// On va réveiller le premier thread dans la waiting_queue (si existant).
	Thread* t = (Thread *) waiting_queue->getFirst()->item;
	waiting_queue->Remove();
	// Avant de passer à l'autre thread, on restaure l'état des interruptions.
