		  ...
		  int64_t func, int64_t arg)
{
  ...
  #ifdef ETUDIANTS_TP
	process = owner; // Attach the thread to the process it will be running in.

	// Initialize thread content.
	// Allocate the user stack.
	int sp = process->addrspace->StackAllocate();
	InitThreadContext(0, sp, arg);

	// Allocate the stack for the RISC-V simulator.
	int8_t* sim_sp = AllocBoundedArray(SIMULATORSTACKSIZE);
	InitSimulatorContext(sim_sp, SIMULATORSTACKSIZE);

	// Increment the number of threads for the process and mark it ready.
...
Thread::Finish()
{
  ...
  #ifdef ETUDIANTS_TP
	// We mark this current thread to be destroyed.
...
/*!	Saves the CPU state of a user program on a context switch
*/
//----------------------------------------------------------------------
void
Thread::SaveProcessorState()
{
  ...
  #ifdef ETUDIANTS_TP
	// Sauvegarder le contexte du thread (thread_context) [la stack est déjà sauvegardée]
