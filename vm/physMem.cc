...
PhysicalMemManager::~PhysicalMemManager() {
  ...
  while (!free_page_list.IsEmpty()) free_page_list.Remove();
...
*/
//-----------------------------------------------------------------
int PhysicalMemManager::AddPhysicalToVirtualMapping(AddrSpace* owner, uint64_t virtualPage) 
{
  #ifndef ETUDIANTS_TP
  ...
  exit(ERROR);
	...
	if (page == INVALID_PAGE) page = EvictPage();
	tpr[page].free = false;
	//tpr[page].locked = false; ?
...
int PhysicalMemManager::EvictPage() {
  ...
  #ifdef ETUDIANTS_TP
	int locked_pages = 0;
	for (; ; i_clock = (i_clock + 1) % g_cfg->NumPhysPages) {
		struct tpr_c page = tpr[i_clock];
		if (!page.free) {
			page.free = true;
		} else if (!page.locked) {
			// réquisition page
...
void PhysicalMemManager::Print(void) {
  ...
  for (i=0;i<g_cfg->NumPhysPages;i++) {
    printf("Page %" PRIu64 " free=%d locked=%d virtpage=%" PRIu64 " owner=%lx U=%d M=%d\n",
	   i,
	   tpr[i].free,
	   tpr[i].locked,
	   tpr[i].virtualPage,
